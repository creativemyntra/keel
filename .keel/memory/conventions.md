...
- 2026-07-17 (KEEL-105): HTTP rejection responses (guard-path 4xx) use the constant-body contract (ADR-004 D-3): compile-time-constant plain-text body, `text/plain; charset=utf-8`, `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`, zero request-data echo, zero fs I/O; guards run before routing.
- 2026-07-17 (KEEL-105): Dashboard `--port` flag syntax is per surface: `scripts/keel-dashboard.cjs` parses space-separated `--port <N>` only; the `bin/keel.js` wrapper parses `--port=<N>` only (wrong form silently falls back to 7772) — docs must state the form for the surface being shown.
