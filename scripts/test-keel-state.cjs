...
`;
    ...
    const phase3 = {
      ...
      component_contract: [
        { component: 'Button', data_testid: 'btn-submit', states: ['default'], aria: { role: 'button' } },
      ],
  ...
  {
    // Test 1: Phase 3 without component_contract -> FAIL
    const cwd1 = makeTmpDir('ui3-no-contract');
    engine(cwd1, 'init', 'UI3-NO', '--title', 'no contract');
    const phaseDir1 = path.join(cwd1, '.keel', 'state', 'UI3-NO');
    fs.mkdirSync(phaseDir1, { recursive: true });
    const phase3NoContract = {
      phase: 3, agent: 'ui-designer', story_id: 'UI3-NO', confidence: 'high',
      findings: ['Design complete'], acceptance_criteria_ids: ['AC-1'],
      decisions: [], artifacts: [], next_phase: 4, blockers: [],
      design_review_checklist: {
        story_alignment: true, wcag_2_1_aa: true, responsive_design: true,
        design_tokens: true, palette_typography: true,
      },
      // Missing component_contract
    };
    fs.writeFileSync(path.join(phaseDir1, '03-ui-designer.json'), JSON.stringify(phase3NoContract));
    const r1 = engine(cwd1, 'validate', 'UI3-NO', '03-ui-designer.json');
    assert('UI-3: phase 3 without component_contract FAILS', r1.code !== 0 && /component_contract/.test(r1.out),
      `code=${r1.code} msg=${r1.out.slice(0, 160)}`);
  }

  {
    // Test 2: Phase 3 with valid component_contract -> PASS
    const cwd2 = makeTmpDir('ui3-valid-contract');
    engine(cwd2, 'init', 'UI3-VALID', '--title', 'valid contract');
    const phaseDir2 = path.join(cwd2, '.keel', 'state', 'UI3-VALID');
    fs.mkdirSync(phaseDir2, { recursive: true });
    const phase3Valid = {
      phase: 3, agent: 'ui-designer', story_id: 'UI3-VALID', confidence: 'high',
      findings: ['Design complete'], acceptance_criteria_ids: ['AC-1'],
      decisions: [], artifacts: [], next_phase: 4, blockers: [],
      design_review_checklist: {
        story_alignment: true, wcag_2_1_aa: true, responsive_design: true,
      ...
      component_contract: [
        {
          ...
          aria: { role: 'button', type: 'submit' },
        },
        {
          component: 'Form Input',
          data_testid: 'input-email',
          states: ['default', 'focus', 'error'],
          aria: { role: 'textbox', aria_label: 'Email address' },
        },
      ],
    };
    fs.writeFileSync(path.join(phaseDir2, '03-ui-designer.json'), JSON.stringify(phase3Valid));
    const r2 = engine(cwd2, 'validate', 'UI3-VALID', '03-ui-designer.json');
    assert('UI-3: valid component_contract PASSES', r2.code === 0,
      `code=${r2.code} msg=${r2.out.slice(0, 160)}`);
  }

  {
    // Test 3: Phase 3 with duplicate testids -> FAIL
    const cwd3 = makeTmpDir('ui3-dup-testid');
    engine(cwd3, 'init', 'UI3-DUP', '--title', 'duplicate testid');
    const phaseDir3 = path.join(cwd3, '.keel', 'state', 'UI3-DUP');
    fs.mkdirSync(phaseDir3, { recursive: true });
    const phase3Dup = {
      phase: 3, agent: 'ui-designer', story_id: 'UI3-DUP', confidence: 'high',
      findings: ['Design complete'], acceptance_criteria_ids: ['AC-1'],
      decisions: [], artifacts: [], next_phase: 4, blockers: [],
      design_review_checklist: {
        story_alignment: true, wcag_2_1_aa: true, responsive_design: true,
      ...
      component_contract: [
        { component: 'Button 1', data_testid: 'btn-submit', states: ['default'], aria: { role: 'button' } },
        { component: 'Button 2', data_testid: 'btn-submit', states: ['default'], aria: { role: 'button' } }, // DUPLICATE
      ],
    };
    fs.writeFileSync(path.join(phaseDir3, '03-ui-designer.json'), JSON.stringify(phase3Dup));
    const r3 = engine(cwd3, 'validate', 'UI3-DUP', '03-ui-designer.json');
    assert('UI-3: duplicate testids FAIL', r3.code !== 0 && /duplicate/.test(r3.out),
      `code=${r3.code} msg=${r3.out.slice(0, 160)}`);
  }

  {
    // Test 4: Phase 3 with non-kebab-case testid -> FAIL
    const cwd4 = makeTmpDir('ui3-bad-case');
    engine(cwd4, 'init', 'UI3-BAD', '--title', 'bad testid case');
    const phaseDir4 = path.join(cwd4, '.keel', 'state', 'UI3-BAD');
    fs.mkdirSync(phaseDir4, { recursive: true });
    const phase3Bad = {
      phase: 3, agent: 'ui-designer', story_id: 'UI3-BAD', confidence: 'high',
      findings: ['Design complete'], acceptance_criteria_ids: ['AC-1'],
      decisions: [], artifacts: [], next_phase: 4, blockers: [],
      design_review_checklist: {
        story_alignment: true, wcag_2_1_aa: true, responsive_design: true,
      ...
      component_contract: [
        { component: 'Button', data_testid: 'btnSubmit', states: ['default'], aria: { role: 'button' } }, // NOT kebab-case
      ],
    };
    fs.writeFileSync(path.join(phaseDir4, '03-ui-designer.json'), JSON.stringify(phase3Bad));
    const r4 = engine(cwd4, 'validate', 'UI3-BAD', '03-ui-designer.json');
    assert('UI-3: non-kebab-case testid FAILS', r4.code !== 0 && /kebab-case/.test(r4.out),
      `code=${r4.code} msg=${r4.out.slice(0, 160)}`);
  }
