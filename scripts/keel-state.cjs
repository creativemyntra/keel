...
function cmdSetupVcs(args) {
  ...
  // Write vcs.yml
  const content = `# Keel VCS Provider Configuration (T19)
...
// ------------------------------------------------------------------- approve-phase (T6 + T19)
// Record VCS PR approval for a phase (T19: provider-agnostic, T6: design review).
// Approval is verified against configured VCS (GitHub, Bitbucket, etc.), server-side.
// VCS provider + repo resolved exclusively from .keel/vcs.yml (fail-closed if missing).
// Usage: approve-phase <story-id> <phase-number> --via-pr <PR-number>

function cmdApprovePhase(args) {
  const storyId = args[0];
  const phaseStr = args[1];
  const prNumber = flag(args, '--via-pr') || flag(args, '--via-github-pr') || ''; // T19: generic --via-pr
  ...
  // T19: Load VCS config (fail-closed if missing/malformed)
  let vcsConfig;
  try {
    const vcsProviders = require('./lib/vcs-providers.cjs');
    const vcsYmlPath = path.join('.keel', 'vcs.yml');
    vcsConfig = vcsProviders.loadVcsConfig(vcsYmlPath);
  } catch (err) {
    die(2, `HALT: VCS configuration error — ${err.message}\nInitialize with: keel setup vcs`);
  }

  // T19: Query approval from configured VCS provider using gh CLI (works for GitHub, Bitbucket Cloud via GH_TOKEN)
  try {
    const { execSync } = require('child_process');
    let prApiPath = `repos/${vcsConfig.owner}/${vcsConfig.repo}/pulls/${prNumber}`;
    let reviewsApiPath = `repos/${vcsConfig.owner}/${vcsConfig.repo}/pulls/${prNumber}/reviews`;

    // For Bitbucket Cloud, use different API structure (requires gh CLI Bitbucket support or fallback to curl)
    if (vcsConfig.provider === 'bitbucket') {
      // Bitbucket Cloud: use v2 REST API (not directly supported by gh CLI, so use curl with token from config)
      // RH-1 FIX: Load token from .keel/vcs.yml token_file, not env var
      const vcsProviders = require('./lib/vcs-providers.cjs');
      const token = vcsProviders.loadAuthToken(vcsConfig.token_file) || process.env.BITBUCKET_TOKEN || '';
      if (!token) {
        die(1, `Bitbucket approval requires token at ~/.keel/secrets/bitbucket.token or BITBUCKET_TOKEN env var`);
      }
      try {
        const prUrl = `https://api.bitbucket.org/2.0/repositories/${vcsConfig.owner}/${vcsConfig.repo}/pullrequests/${prNumber}`;
        // RH-3 FIX: Use Bearer token (Bitbucket Cloud v2.0 standard auth)
        const curlCmd = `curl -s -H "Authorization: Bearer ${token}" "${prUrl}" 2>/dev/null`;
        const prData = execSync(curlCmd, { encoding: 'utf8' });
        let pr;
        try {
          pr = JSON.parse(prData);
        } catch (e) {
          die(1, `Bitbucket API returned invalid JSON for PR #${prNumber}: ${prData.slice(0, 100)}`);
        }
        // RH-3 FIX: Correct field path for Bitbucket v2.0 API
        // v2.0: PR object has 'reviewers' array with approved boolean
        const approvals = (pr.reviewers || []).filter((r) => r.approved === true).length;
        if (approvals === 0) {
          die(1, `Bitbucket PR #${prNumber} has no approvals. Request review from a team member.`);
        }

        // RH-4 FIX: Verify PR head branch matches story (prevent unrelated PRs from approving)
        const prHeadBranch = pr.source && pr.source.branch ? pr.source.branch.name : null;
        if (prHeadBranch) {
          try {
            verifyPrBelongsToStory(storyId, prHeadBranch);
          } catch (err) {
            die(1, `PR validation failed: ${err.message}`);
          }
        }

        // Continue with approval recording
        recordPhaseApproval(manifest, storyId, phase, prNumber, approvals, vcsConfig, prHeadBranch);
      } catch (err) {
        // RH-5 FIX: Redact token from error message
        const safeMsg = err.message.replace(new RegExp(token, 'g'), '***TOKEN***');
        die(1, `failed to query Bitbucket PR #${prNumber}: ${safeMsg}`);
      }
    } else if (vcsConfig.provider === 'github' || vcsConfig.provider === 'github-enterprise') {
      // GitHub: use gh CLI (token from GH_TOKEN env or ~/.config/gh/hosts.yml, or from .keel/secrets/github.token)
      // RH-1 FIX: Load token from config file if gh CLI token not already set
      const vcsProviders = require('./lib/vcs-providers.cjs');
      const ghToken = vcsProviders.loadAuthToken(vcsConfig.token_file);

      const ghEnv = Object.assign({}, process.env);
      if (ghToken) {
        ghEnv.GH_TOKEN = ghToken;
      }

      const ghCmd = vcsConfig.base_url ? `gh api ${prApiPath} --hostname ${vcsConfig.base_url}` : `gh api ${prApiPath}`;
      const reviewsCmd = vcsConfig.base_url ? `gh api ${reviewsApiPath} --jq '[.[] | select(.state == "APPROVED")] | length' --hostname ${vcsConfig.base_url}` : `gh api ${reviewsApiPath} --jq '[.[] | select(.state == "APPROVED")] | length'`;

      try {
        // Fetch PR to get branch info (for RH-4 branch matching)
        const prDataCmd = vcsConfig.base_url ? `gh api ${prApiPath} --hostname ${vcsConfig.base_url}` : `gh api ${prApiPath}`;
        const prDataJson = execSync(prDataCmd, { encoding: 'utf8', stdio: 'pipe', env: ghEnv });
        let prData;
        try {
          prData = JSON.parse(prDataJson);
        } catch (e) {
          die(1, `GitHub API returned invalid JSON for PR #${prNumber}: ${prDataJson.slice(0, 100)}`);
        }

        const reviewData = execSync(reviewsCmd, { encoding: 'utf8', stdio: 'pipe', env: ghEnv });
        const approvalCount = parseInt(reviewData.trim(), 10) || 0;

        if (approvalCount === 0) {
          die(1, `GitHub PR #${prNumber} has no approvals. Request review from a team member.`);
        }

        // RH-4 FIX: Verify PR head branch matches story (prevent unrelated PRs from approving)
        const prHeadBranch = prData.head && prData.head.ref ? prData.head.ref : null;
        if (prHeadBranch) {
          try {
            verifyPrBelongsToStory(storyId, prHeadBranch);
          } catch (err) {
        ...
        recordPhaseApproval(manifest, storyId, phase, prNumber, approvalCount, vcsConfig, prHeadBranch);
      } catch (err) {
        die(1, `failed to query GitHub PR #${prNumber}: ${err.message}`);
      }
    } else {
      die(1, `VCS provider ${vcsConfig.provider} not yet supported — open an issue for Bitbucket Server/Data Center support`);
    }
  } catch (err) {
    if (err.message.includes('VCS provider')) throw err; // Re-throw known error
    die(1, `VCS approval query failed: ${err.message}`);
  }
}
...
function recordPhaseApproval(manifest, storyId, phase, prNumber, approvalCount, vcsConfig, prHeadBranch = null) {
  ...
  manifest.approved_phases[String(phase)] = {
    ...
    vcs_provider: vcsConfig.provider,  // T19: track which provider approved this
    pr_head_branch: prHeadBranch || null,  // RH-4: track branch for audit
  };
  ...
  appendAudit(storyId, {
    ...
    approver_count: approvalCount,
    content_hash: contentHash,
    vcs_provider: vcsConfig.provider,
    pr_head_branch: prHeadBranch || null,  // RH-4: track for audit trail
    vcs_config: { provider: vcsConfig.provider, owner: vcsConfig.owner, repo: vcsConfig.repo }
  });

  console.log(`OK: phase ${phase} approved via ${vcsConfig.provider} PR #${prNumber} (${approvalCount} approval(s))`);
