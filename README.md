...
[x] **15 Specialized Agents** -- 10 pipeline phase + 2 meta/support + 3 infrastructure agents  
[x] **10-Phase Pipeline** -- dedicated UI design (3), code + unit tests in one phase (5), and Playwright E2E (7) phases; defect express lane runs 1->5->6->8  
...
[x] **Execution-Verified Gates** -- the handshake gate re-runs tests instead of trusting claims (anti-hallucination)  
...
[x] **Audit Trail** -- per-story JSONL log supporting your compliance evidence process  
[x] **Unit Testing in Phase 5** -- software-engineer writes code and unit tests in one phase; coverage >= 80% gated before QA  
[x] **Coverage Gate** -- >=80% enforced before the QA phase  
[x] **No Patch Development** -- defect fixes require an RCA + revert-checked regression test; symptom patches fail the gate  
[x] **Security Phase** -- OWASP Top 10 review + layered SAST/SCA: PHPStan & composer audit always, SonarQube & Snyk when configured  
[x] **Stack: CakePHP 4.4/PHP 8.1** -- production-proven; multi-stack support in a future release  
...
- **Prescan hardening** -- `snyk` scanner now skips with "not applicable" on dirs with no supported project manifest. Previously reported "not configured" on manifest-free repos.
- **Test env isolation** -- `composer-audit` prescan test strips PATH to system-only entries so it passes on any host regardless of installed tools.
- **Version bump** -- 3.16.0 -> 3.16.1 across `package.json`, `bin/keel.js`, and plugin files.
...
- Register `/keel:*` commands, 15 agents, and 11 skills
...
[x] **Layered SAST** -- PHPStan baseline always; SonarQube quality gate when configured (gate ERROR = release blocker)  
[x] **Layered SCA** -- composer/npm audit baseline always; Snyk when configured (high/critical = release blocker)  
...
[x] **Secrets hygiene** -- no API keys in git (`~/.keel/secrets/`, gitignored); agents are forbidden from outputting credentials, tokens, or PII
